...
// Discogs serves localised URLs too, e.g. https://www.discogs.com/fr/release/1234567-Title,
// so the page type can sit behind an optional locale segment.
const DISCOGS_LOCALE_SEGMENT = '(?:[a-z]{2}(?:-[A-Za-z]{2})?\\/)?';
...
function parseDiscogsUrl(url) {
  ...
  // https://www.discogs.com/fr/release/1234567-Artist-Title
  const releaseMatch = url.match(
    new RegExp(`discogs\\.com\\/${DISCOGS_LOCALE_SEGMENT}(release|master)\\/(\\d+)`, 'i')
  );
  if (releaseMatch) {
...
// Discogs pages that have no tracklist for the tagger to read, named the way a
// person would describe them.
const DISCOGS_PAGE_TYPES = {
  label: 'a label page',
  artist: 'an artist page',
  list: 'a list page',
  lists: 'a list page',
  user: 'a user profile',
  seller: 'a seller page',
  sell: 'a Marketplace listing',
  search: 'a search results page',
  forum: 'a forum page',
  group: 'a group page'
};

// Explains why a Discogs URL that isn't a release or master can't be used.
// Returns null for anything that isn't a Discogs link at all.
function describeUnsupportedDiscogsUrl(url) {
  if (!/discogs\.com/i.test(url)) return null;

  const typeMatch = url.match(
    new RegExp(`discogs\\.com\\/${DISCOGS_LOCALE_SEGMENT}([a-z]+)`, 'i')
  );
  const pageType = typeMatch ? typeMatch[1].toLowerCase() : '';
  const described = DISCOGS_PAGE_TYPES[pageType]
    || (pageType ? `a Discogs ${pageType} page` : 'a Discogs page');

  return `That link is ${described}, which has no tracklist for the tagger to read. `
    + 'Open the specific album you want on Discogs and paste its release or master URL instead '
    + '— those look like https://www.discogs.com/release/1234567-Artist-Title or '
    + 'https://www.discogs.com/master/7654321-Artist-Title.';
}
...
export default function TaggerPage({ initialUrl }) {
  ...
  // Add CUE export states
  const [cueExportState, setCueExportState] = useState('idle'); // idle | copied | hover
  const [cueFileContent, setCueFileContent] = useState('');
  ...
  useEffect(() => {
    if (initialUrl) {
      ...
      // Pass the URL directly to the submit function to avoid state timing issues
      handleUrlSubmit(null, initialUrl);
    }
  }, [initialUrl]);
  ...
  const handleUrlSubmit = async (e, directUrl) => {
    ...
    // Not a release or master. Say why instead of silently doing nothing, which is
    // what a label/artist/list URL used to get.
    const unsupportedDiscogsMessage = describeUnsupportedDiscogsUrl(urlToSubmit);
    if (unsupportedDiscogsMessage) {
      setDiscogsResponse(null);
      setSubmitStatus('error');
      setSubmitMessage('That Discogs page has no tracklist');
      setDiscogsError(unsupportedDiscogsMessage);
      return;
    }
    ...
    if (urlToSubmit && urlToSubmit.trim()) {
      setDiscogsResponse(null);
      setSubmitStatus('error');
      setSubmitMessage('Not a Discogs URL');
      setDiscogsError(
        'The tagger reads tracklists from Discogs. Paste a Discogs release or master URL '
        + '— for example https://www.discogs.com/release/1234567-Artist-Title.'
      );
    }
  };
  ...
  useEffect(() => {
    ...
    if (inputSource === 'files' && audioFilesRef.current.length > 0) {
      ...
      const showTimes = inputSources.files.times;
      const lines = markers.map((marker, index) => {
        const start = showTimes ? marker.formattedTime : '00:00';
        const end = showTimes ? (index < markers.length - 1 
          ? markers[index + 1].formattedTime 
          : formatTime(marker.startTime + 60)) : '00:00'; // Default 1 minute duration for last track
  ...
  // Every copy button goes through here.
  //
  // navigator.clipboard.writeText is not always available or allowed: it needs
  // a secure context, and extensions can hook it — uBlock Origin 1.73 added
  // clipboard-write blocking for ClickFix attacks, and a page that writes
  // multi-line text on a click can trip it. When the write is refused, the
  // execCommand path still works, because it copies an actual DOM selection
  // rather than calling the clipboard API.
  //
  // Returns whether the text really reached the clipboard, so the button can
  // stop claiming "Copied!" over a write that silently failed — which is what
  // it did for every one of these handlers before.
  const copyToClipboard = async (text) => {
    ...
    if (navigator.clipboard?.writeText) {
      try {
        ...
        console.warn('[tagger] clipboard API write refused, falling back to execCommand:', err);
      }
    }

    // Off-screen textarea + execCommand. Deprecated, but it is the only path
    // that works without the async clipboard API, and it is a different
    // mechanism from the one blockers hook.
    try {
      ...
      console.warn('[tagger] execCommand copy failed:', err);
    }
    return false;
  };

  // Shared by the buttons: flips to "copied" only on a real copy, and says so
  // when something blocked it instead of pretending it worked.
  const runCopy = async (text, setState, resetMs = 900) => {
    const ok = await copyToClipboard(text);
    setState(ok ? 'copied' : 'failed');
    setTimeout(() => setState('idle'), ok ? resetMs : 4000);
    return ok;
  };

  const handleCopy = () => runCopy(
    inputValue ? `Timestamps generated by https://tagger.site:\n${inputValue}` : '',
    setCopyState,
  );

  // Handle tags copy
  const handleTagsCopy = () => runCopy(tagsValue, setTagsCopyState);

  // Handle hashtags copy
  const handleHashtagsCopy = () => runCopy(hashtagsValue, setHashtagsCopyState);

  // Generate CUE file content from ALS markers and filenames
  const generateCueFile = () => {
    if (!inputSources.files.data || inputSources.files.data.type !== 'als') {
      return '';
    }

    const markers = inputSources.files.data.markers || [];
    const filenames = inputSources.alsFilenames.data?.filenames || [];
    const albumTitle = discogsResponse?.title || 'Unknown Album';
    const performer = discogsResponse?.artists?.[0]?.name || 'Unknown Artist';

    if (markers.length === 0) {
      return '';
    }

    let cueContent = `REM Generated by tagger.site\n`;
    cueContent += `TITLE "${albumTitle}"\n`;
    cueContent += `PERFORMER "${performer}"\n`;
    cueContent += `FILE "${filenames[0] || 'audio.wav'}" WAVE\n`;

    markers.forEach((marker, index) => {
      const trackNumber = index + 1;
      const trackTitle = marker.name || `Track ${trackNumber}`;
      
      // Convert seconds to CUE time format (MM:SS:FF where FF is frames)
      const minutes = Math.floor(marker.startTime / 60);
      const seconds = Math.floor(marker.startTime % 60);
      const frames = Math.floor((marker.startTime % 1) * 75); // 75 frames per second
      
      const timeString = `${minutes.toString().padStart(2, '0')}:${seconds.toString().padStart(2, '0')}:${frames.toString().padStart(2, '0')}`;
      
      cueContent += `  TRACK ${trackNumber.toString().padStart(2, '0')} AUDIO\n`;
      cueContent += `    TITLE "${trackTitle}"\n`;
      cueContent += `    PERFORMER "${performer}"\n`;
      cueContent += `    INDEX 01 ${timeString}\n`;
    });

    return cueContent;
  };

  // Handle CUE file copy
  const handleCueCopy = () => {
    const cueContent = generateCueFile();
    if (cueContent) runCopy(cueContent, setCueExportState, 2000);
  };

  // Handle CUE file download
  const handleCueDownload = () => {
    const cueContent = generateCueFile();
    if (cueContent) {
      const albumTitle = discogsResponse?.title || 'Unknown Album';
      const filename = `${albumTitle.replace(/[^a-zA-Z0-9]/g, '_')}.cue`;
  ...
  const generateCombinedTimestamps = () => {
    ...
    if (useFileTimes && filesSource.data) {
      ...
      timingData = {
        ...
        // Use the durations and times from discogsDurationsRef, which are set by the main useEffect
        durations: discogsDurationsRef.current
      };
    }
    ...
    // Handle case where only ALS filenames are available without timing data
    if (!timingData && useAlsFilenamesMetadata && alsFilenamesSource.data && !inputSources.alsFilenames.times) {
      // Generate lines with only filenames and zero times
      const lines = alsFilenamesSource.data.filenames.map(filename => {
        const lineData = formatOrder
          .map(item => {
            if (item.value === 'blank') return '';
            if (item.value === 'startTime') return '00:00';
            if (item.value === 'endTime') return '00:00';
            if (item.value === 'title') return filename;
            if (item.value === 'dash') return '-';
            if (item.value === 'dash-artist') return '';
            if (item.value === 'artist') return '';
            return '';
    ...
    if (timingData.type === 'als') {
      ...
      timingData.markers.forEach((marker, index) => {
        // Check if times should be shown based on the times toggle
        const showTimes = inputSources.files.times;
        const start = showTimes ? marker.formattedTime : '00:00';
        const end = showTimes ? (index < timingData.markers.length - 1 
      ...
      timingData.tracks.forEach((track, idx) => {
        // Use durations from discogsDurationsRef.current, which matches the main useEffect
  ...
  // Generate CUE file content when input sources change
  useEffect(() => {
    const cueContent = generateCueFile();
    setCueFileContent(cueContent);
                ...
                if (inputSources.url.data || inputSources.files.data) {
                  ...
                  if (inputSource === 'files' && audioFilesRef.current.length > 0) {
                    ...
                    const lines = audioFilesRef.current.map((file, idx2) => {
                      ...
                      const title = file.name.replace(/\.[^/.]+$/, '');
                      // Unique log for this context
                      console.log(`[TaggerPage/useEffect:formatOrder+artistDisabled+inputSource] Processing file: ${file.name}, duration: ${durationsRef.current[idx2]}s, start: ${start}, end: ${end}`);
...
18:18 - 21:03 Imprevu`
        ...
        <button
          style={{
            ...
            borderRadius: '4px',
            border: `1px solid ${t.border}`,
            background: copyState === 'copied' ? '#ffe156' : copyState === 'failed' ? '#fed7d7' : t.buttonBg,
            color: copyState === 'copied' || copyState === 'failed' ? '#000000' : t.text,
            fontWeight: 600,
            cursor: 'pointer',
        ...
        >
          {copyState === 'copied' ? 'Copied!'
            : copyState === 'failed' ? 'Copy blocked — select the text and press Ctrl+C'
            : `Copy ${(inputValue ? (`Timestamps generated by https://tagger.site:\n${inputValue}`) : '').length} chars to clipboard`}
        </button>

        {/* CUE Export Buttons - only show when ALS data is available */}
        {inputSources.files.data && inputSources.files.data.type === 'als' && cueFileContent && (
          <div style={{ marginBottom: '1rem' }}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
              <button
                type="button"
                onClick={handleCueCopy}
                style={{
                  padding: '0.5rem 1rem',
                  borderRadius: '4px',
                  border: `1px solid ${t.border}`,
                  background: cueExportState === 'copied' ? '#ffe156' : cueExportState === 'failed' ? '#fed7d7' : t.buttonBg,
                  fontWeight: 600,
                  cursor: 'pointer',
                  boxShadow: '0 1px 4px rgba(0,0,0,0.04)',
                  transition: 'background 0.2s, box-shadow 0.2s, color 0.2s',
              ...
              >
                {cueExportState === 'copied' ? 'Copied!' : cueExportState === 'failed' ? 'Copy blocked' : 'Copy CUE'}
              </button>
              
              <button
                type="button"
                onClick={handleCueDownload}
                style={{
                  padding: '0.5rem 1rem',
                  borderRadius: '4px',
                  border: `1px solid ${t.border}`,
      ...
      <button
        style={{
          ...
          borderRadius: '4px',
          border: `1px solid ${t.border}`,
          background: tagsCopyState === 'copied' ? '#ffe156' : tagsCopyState === 'failed' ? '#fed7d7' : t.buttonBg,
          color: tagsCopyState === 'copied' || tagsCopyState === 'failed' ? '#000000' : t.text,
          fontWeight: 600,
          cursor: 'pointer',
          marginBottom: '0.5rem',
          display: 'block',
      ...
      >
        {tagsCopyState === 'copied' ? 'Copied!'
          : tagsCopyState === 'failed' ? 'Copy blocked — select the text and press Ctrl+C'
      ...
      <div>
        ...
        <button
          style={{
            ...
            borderRadius: '4px',
            border: `1px solid ${t.border}`,
            background: hashtagsCopyState === 'copied' ? '#ffe156' : hashtagsCopyState === 'failed' ? '#fed7d7' : t.buttonBg,
            color: hashtagsCopyState === 'copied' || hashtagsCopyState === 'failed' ? '#000000' : t.text,
            fontWeight: 600,
            cursor: 'pointer',
            marginBottom: '0.5rem',
            display: 'block',
        ...
        >
          {hashtagsCopyState === 'copied' ? 'Copied!'
            : hashtagsCopyState === 'failed' ? 'Copy blocked — select the text and press Ctrl+C'
