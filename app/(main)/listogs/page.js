...
function DiscogsAuthTestPageInner() {
  ...
  const [retryReason, setRetryReason] = useState('rate-limit'); // 'rate-limit' | 'server-error' | 'network'
  ...
  const handleInputChange = (value) => {
    ...
    const masterMatch = value.match(/discogs\.com\/master\/(\d+)/);
    if (masterMatch && masterMatch[1]) {
      setExtractedId(masterMatch[1]);
      setSelectedType('master');
      storeListogsData(results, value, masterMatch[1], 'master');
      return;
    }
    ...
    // Try matching master bracket format
    const bracketMasterMatch = value.match(/^\[m(\d+)\]$/);
    if (bracketMasterMatch && bracketMasterMatch[1]) {
      setExtractedId(bracketMasterMatch[1]);
      setSelectedType('master');
      storeListogsData(results, value, bracketMasterMatch[1], 'master');
      return;
    }
  ...
  const autoPopulatePlaylistData = (searchId = null) => {
    ...
    // Extract name and type from URL if we have a URL but no name
    let discogsType = 'artist'; // default
    if (discogsUrl !== 'N/A') {
      const labelMatch = discogsUrl.match(/discogs\.com\/label\/(\d+)-(.+)/);
      const artistMatch = discogsUrl.match(/discogs\.com\/artist\/(\d+)-(.+)/);
      const masterMatch = discogsUrl.match(/discogs\.com\/master\/(\d+)-(.+)/);
      if (labelMatch) {
        discogsType = 'label';
        if (discogsName === 'Unknown') {
          discogsName = decodeURIComponent(labelMatch[2])
            .replace(/-/g, ' ')
            .replace(/\b\w/g, l => l.toUpperCase());
          console.log('Extracted label name from URL:', discogsName);
        }
      } else if (masterMatch) {
        discogsType = 'master';
        if (discogsName === 'Unknown') {
          discogsName = decodeURIComponent(masterMatch[2])
            .replace(/-/g, ' ')
            .replace(/\b\w/g, l => l.toUpperCase());
          console.log('Extracted master name from URL:', discogsName);
        }
      } else if (artistMatch && discogsName === 'Unknown') {
        discogsName = decodeURIComponent(artistMatch[2])
          .replace(/-/g, ' ')
          .replace(/\b\w/g, l => l.toUpperCase());
        console.log('Extracted artist name from URL:', discogsName);
      }
    }
    ...
    // Create title — include type context for labels and masters
    const typeLabel = discogsType === 'label' ? ' (Label)' : discogsType === 'master' ? ' (Master)' : '';
    const title = `${discogsName}${typeLabel} - All Discogs Videos [Listogs]`;

    // Create description with proper URL
    const sourceLabel = discogsType === 'label' ? 'Discogs Label'
      : discogsType === 'master' ? 'Discogs Master Release'
      : 'Discogs Artist';
    let description = `Curated playlist from Listogs search results.

🔗 Generated by: https://www.martinbarker.me/listogs
📀 Original ${sourceLabel}: ${discogsUrl}`;
    ...
    try {
      ...
      if (useExistingPlaylist) {
        ...
        const response = await fetch(`${apiBaseURL}/youtube/createPlaylist`, {
          ...
          credentials: 'include',
          body: JSON.stringify({ ...playlistData, tokens })
        });
        ...
        if (response.ok) {
          ...
          const detail = errorData.reason ? ` (${errorData.reason})` : '';
          setYoutubeError(errorData.error || `Failed to create playlist (HTTP ${response.status})${detail}`);
          setPlaylistLoading(false);
          return;
        }
      }
  ...
  const addVideosToPlaylist = async (playlistId, videoIds, tokens) => {
    try {
      ...
      const NON_RETRYABLE_REASONS = [
        'videonotfound', 'videoidrequired', 'invalidvideoid', 'videonoteditable',
        'playlistitemsnotaccessible', 'failedprecondition', 'manualsortrequired',
        'notfound', 'forbidden'
      ];
      const isNonRetryableVideoError = (status, errorData) => {
        if (status === 404 || status === 400) return true;
        const reason = (errorData?.reason || '').toString().toLowerCase();
        const message = (errorData?.error || '').toString().toLowerCase();
        return NON_RETRYABLE_REASONS.some(r => reason.includes(r) || message.includes(r));
      };
      ...
      for (let i = 0; i < uniqueVideoIds.length; i++) {
        ...
        while (!success) {
          try {
            ...
            // Clear retry UI on success after any backoff (rate limit or server error)
            if (rateLimitRetries > 0 || serverErrorRetries > 0) {
              setRateLimited(false);
              setRetryAfter(null);
              setRetryAttempt(0);
              setYoutubeError('');
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, maxWidth: '480px' }}>
          <input
            ...
            placeholder="Enter a Discogs URL (artist, label, master, release, or list)"
        ...
        {discogsInput && !selectedType && (
          <div style={{ color: 'red', marginTop: 8 }}>
            Not a valid Discogs artist, label, master, release, or list URL.
          </div>
        )}
          ...
          <div style={{
            ...
            {youtubeError && (
              <div style={{
                ...
                <span style={{ color: t.text }}>
                  {rateLimited
                    ? (retryReason === 'rate-limit' ? '⚠️ Rate Limited: ' : retryReason === 'network' ? '📡 Network Issue: ' : '🛠️ Server Error: ')
                    : 'Error: '}{youtubeError}
                </span>
                  ...
                  <div style={{ marginTop: 8, fontSize: 14, color: t.text }}>
                    ...
                    <div style={{ marginTop: 4, fontSize: 12, color: t.textSecondary }}>
                      {retryReason === 'rate-limit'
                        ? `Attempt ${retryAttempt} — YouTube API rate limit hit. Will keep retrying automatically.`
                        : retryReason === 'network'
                          ? `Attempt ${retryAttempt} — network connection issue. Will keep retrying until it succeeds.`
                          : `Attempt ${retryAttempt} — YouTube server returned an error. Will keep retrying until it succeeds.`}
                    </div>
                    <div style={{
                      marginTop: 8,
                      height: 6,
